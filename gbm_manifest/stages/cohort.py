...
class CohortSelector:
    ...
    def run(self, manifest: pd.DataFrame, force: bool = False) -> pd.DataFrame:
        ...
        # Step 8: assign partition (UPENN test set vs training)
        df["partition"] = "train"
        df.loc[df["dataset"] == "upenn_gbm", "partition"] = "external_test"

        log.info("cohort: selected %d sessions (train=%d, test=%d)",
                 len(df),
